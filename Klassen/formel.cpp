...
bool Formel::ist_einfache_Berechnung(const QString text)
{
    ...
    {
        if((text.at(i) == '+' || text.at(i) == '-' || text.at(i) == '*' || text.at(i) == '/') &&
           (text.at(i + 1) == '+' || text.at(i + 1) == '-')) // Wenn z.B. "12+-3"
        {
            anzahl_an_nichtzahlen -= 1;
        }
    }
...
{
    if(zeichen == '0' || zeichen == '1' || zeichen == '2' || zeichen == '2' || zeichen == '3' || zeichen == '4' || zeichen == '5' || zeichen == '6' ||
       zeichen == '7' || zeichen == '8' || zeichen == '9' || zeichen == '.' || zeichen == ',' || zeichen == '+' || zeichen == '-' || zeichen == '*' ||
...
QString Formel::Verschachtelung_text_links(const QString text, const QChar rechenzeichen)
{
    ...
    if(indexBegin - 1 >= 0)
    {
        if(text.at(indexBegin - 1) == '+' || text.at(indexBegin - 1) == '-')
        {
            if(indexBegin - 2 >= 0)
            {
                if(text.at(indexBegin - 2) == '+' || text.at(indexBegin - 2) == '-' || text.at(indexBegin - 2) == '*' ||
                   text.at(indexBegin - 2) == '/')
                {
                    indexBegin -= 1;
                }
            }
...
QString Formel::Verschachtelung_text_rechts(const QString text, const QChar rechenzeichen)
{
    ...
    int indexEnde = text.count();
    // for(int i = index+1 ; ( ist_ziffer(text.at(i)) || text.at(i)=='.' || text.at(i)==',' )  &&  ( text.at(i)!='+' && text.at(i)!='-' &&
    // i<text.count() ) ; i++)
    //{
    //     indexEnde = i;
    // }
    ...
    else
    {
        ...
        else // Wenn die Berechnung noch verschachtelt ist:
        {
            while(!ist_einfache_Berechnung(text))
            {
                ...
                QChar ch = '-';
                while(text.contains(ch) && (text.indexOf(ch) > 0) && (text.at(text.indexOf(ch) - 1) != '+') &&
                      (text.at(text.indexOf(ch) - 1) != '-') && (text.at(text.indexOf(ch) - 1) != '*') && (text.at(text.indexOf(ch) - 1) != '/'))
                {
                    links = Verschachtelung_text_links(text, ch);
                    mitte = Verschachtelung_text_mitte(text, ch);
                    rechts = Verschachtelung_text_rechts(text, ch);
